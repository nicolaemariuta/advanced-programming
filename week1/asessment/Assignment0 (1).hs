...
rotatePoint theta thePoint = Point {x = (x thePoint) * c + (y thePoint) * s, y = (y thePoint) * c - (x thePoint) * s}
...
findMinMaxValues (point1, point2) comparePoint = (Point {x=(min (x point1) (x comparePoint)), y=(min (y point1) (y comparePoint))},
...
bbox points = let (minPoint, maxPoint) = (
